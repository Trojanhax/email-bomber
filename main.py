...
def banner():
    print(Bcolor.GREEN + "+[+[+[ Email-Bomber V1.1 ]+]+]+")
...
4:(Custom) :  <<<: '''))
            if int(self.mode) > int(4) or int(self.mode) < int(1):
                print(Bcolor.RED +'ERROR : Invalid Option. Good Bye')
                sys.exit(1)
...
3 : Outlook >>> : """)
            ...
            self.msg = input(Bcolor.YELLOW + r"""
From    : %s
To      : %s
Subject : %s
          %s """ % (self.formAddr, self.formPwd, self.Subject, self.message))
